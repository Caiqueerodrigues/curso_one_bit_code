...
type Situation = "habitado" | "habitavel" | "inabitavel" | "inexploravel"
...
function createPlanet(name:string, coordenadas:Coord, situation:Situation) { 
    planets.push({
        name,
        coordenadas,
        situation,
...
function attSituation(namePlanet: String, situation:Situation) {
...
function menuOp2() {
    ...
    let sit:Situation = promptValidSituation()
    attSituation(namePlanet, sit)
}

function removeSatOrAddSat(namePlanet:string, nameSat:string, operation:string) {
    planets.forEach(planet => {
        if(planet.name === namePlanet) {
            ...
            if(operation === 'remove') {
                if(planet.satelites.length <= 0) return alert("Não há nenhum satélite cadastrado")
                else {
